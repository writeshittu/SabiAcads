      ...
      <div>
        <Card>
          <Card.Img
            ...
            height={200}
            onClick={() =>
              this.setModalShow(this.props.name, this.props.links[0].url)
            }
          />
          <Card.Body>
          ...
          <Card.Footer>
            <div className="text-right">
              {this.props.subscribed && (
                <button
                  className="pa2 btn-primary pointer o-20"
                  disabled
                  onClick={() => this.props.onSubscribe(this.props.id)}
                >
                  SUBSCRIBE
                </button>
              )}
...
class Courses extends React.Component {
  constructor(props) {
    ...
    axios
      .get(
        `${util.API_BASE_URL}subscriptions/${window.localStorage.getItem(
          "email"
        )}`
      )
      .then((res) => {
        this.setState({
          subscribedCourses:
            res.data.courses !== undefined ? res.data.courses : [],
        });
      });
  }
  ...
  onSubscribe = (courseId) => {
    ...
    axios
        ...
        if (res.data !== undefined && res.data.id !== undefined) {
          ...
          axios
            .put(
              ...
              { courses: currentSubscribedCourses }
            )
            .then((res) => {
      ...
      .catch((err) => {
        ...
        axios
          .post(`${util.API_BASE_URL}subscriptions`, {
            courses: currentSubscribedCourses,
            ...
            {this.props.data.map((data) => (
              <div className="col-sm col-md-3 col-xs mb4" key={data.id}>
                <Cards
                  ...
                  subscribed={
                    this.state.subscribedCourses === undefined
                      ? false
                      : this.state.subscribedCourses.includes(data.id)
                  }
                  id={data.id}
                  links={data.links}
                  image={data.image}
